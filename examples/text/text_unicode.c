...
int main(void)
{
    ...
    RL_InitWindow(screenWidth, screenHeight, "raylib [text] example - unicode");

    // Load the font resources
    // NOTE: fontAsian is for asian languages,
    // fontEmoji is the emojis and fontDefault is used for everything else
    RL_Font fontDefault = RL_LoadFont("resources/dejavu.fnt");
    RL_Font fontAsian = RL_LoadFont("resources/noto_cjk.fnt");
    RL_Font fontEmoji = RL_LoadFont("resources/symbola.fnt");

    RL_Vector2 hoveredPos = { 0.0f, 0.0f };
    RL_Vector2 selectedPos = { 0.0f, 0.0f };
    ...
    while (!RL_WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (RL_IsKeyPressed(KEY_SPACE)) RandomizeEmoji();

        // Set the selected emoji
        if (RL_IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && (hovered != -1) && (hovered != selected))
        {
            selected = hovered;
        ...
        RL_Vector2 position = { 28.8f, 10.0f };
        hovered = -1;
        //----------------------------------------------------------------------------------
        ...
        RL_BeginDrawing();
            ...
            for (int i = 0; i < SIZEOF(emoji); ++i)
            {
                ...
                RL_Rectangle emojiRect = { position.x, position.y, (float)fontEmoji.baseSize, (float)fontEmoji.baseSize };

                if (!RL_CheckCollisionPointRec(mouse, emojiRect))
                {
                    RL_DrawTextEx(fontEmoji, txt, position, (float)fontEmoji.baseSize, 1.0f, selected == i ? emoji[i].color : RL_Fade(RL_LIGHTGRAY, 0.4f));
                }
                else
                {
                    RL_DrawTextEx(fontEmoji, txt, position, (float)fontEmoji.baseSize, 1.0f, emoji[i].color );
                    hovered = i;
                    hoveredPos = position;
                }
            ...
            if (selected != -1)
            {
                ...
                RL_Font *font = &fontDefault;

                // Set correct font for asian languages
                if (RL_TextIsEqual(messages[message].language, "Chinese") ||
                    RL_TextIsEqual(messages[message].language, "Korean") ||
                    RL_TextIsEqual(messages[message].language, "Japanese")) font = &fontAsian;

                // Calculate size for the message box (approximate the height and width)
                RL_Vector2 sz = RL_MeasureTextEx(*font, messages[message].text, (float)font->baseSize, 1.0f);
                ...
                RL_Vector2 a = { selectedPos.x, msgRect.y + msgRect.height }, b = {a.x + 8, a.y + 10}, c= { a.x + 10, a.y };
                ...
                // Draw chat bubble
                RL_DrawRectangleRec(msgRect, emoji[selected].color);
                RL_DrawTriangle(a, b, c, emoji[selected].color);
                ...
                int length = RL_GetCodepointCount(messages[message].text);
                const char *info = RL_TextFormat("%s %u characters %i bytes", messages[message].language, length, size);
                sz = RL_MeasureTextEx(RL_GetFontDefault(), info, 10, 1.0f);
                
                RL_DrawText(info, (int)(textRect.x + textRect.width - sz.x), (int)(msgRect.y + msgRect.height - sz.y - 2), 10, RL_RAYWHITE);
            }
            //------------------------------------------------------------------------------

            // Draw the info text
            RL_DrawText("These emojis have something to tell you, click each to find out!", (screenWidth - 650)/2, screenHeight - 40, 20, RL_GRAY);
            RL_DrawText("Each emoji is a unicode character from a font, not a texture... Press [SPACEBAR] to refresh", (screenWidth - 484)/2, screenHeight - 16, 10, RL_GRAY);
    ...
    // De-Initialization
    //--------------------------------------------------------------------------------------
    RL_UnloadFont(fontDefault);    // Unload font resource
    RL_UnloadFont(fontAsian);      // Unload font resource
    RL_UnloadFont(fontEmoji);      // Unload font resource
...
{
    hovered = selected = -1;
    int start = RL_GetRandomValue(45, 360);

    for (int i = 0; i < SIZEOF(emoji); ++i)
    {
        // 0-179 emoji codepoints (from emoji char array) each 4bytes + null char
        emoji[i].index = RL_GetRandomValue(0, 179)*5;

        // Generate a random color for this emoji
        emoji[i].color = RL_Fade(RL_ColorFromHSV((float)((start*(i + 1))%360), 0.6f, 0.85f), 0.8f);

        // Set a random message for this emoji
        emoji[i].message = RL_GetRandomValue(0, SIZEOF(messages) - 1);
    }
}
