...
void DrawTexturePoly(RL_Texture2D texture, RL_Vector2 center, RL_Vector2 *points, RL_Vector2 *texcoords, int pointCount, RL_Color tint);
...
int main(void)
{
    ...
    RL_InitWindow(screenWidth, screenHeight, "raylib [textures] example - textured polygon");

    // Define texture coordinates to map our texture to poly
    RL_Vector2 texcoords[MAX_POINTS] = {
        (RL_Vector2){ 0.75f, 0.0f },
        (RL_Vector2){ 0.25f, 0.0f },
        (RL_Vector2){ 0.0f, 0.5f },
        (RL_Vector2){ 0.0f, 0.75f },
        (RL_Vector2){ 0.25f, 1.0f},
        (RL_Vector2){ 0.375f, 0.875f},
        (RL_Vector2){ 0.625f, 0.875f},
        (RL_Vector2){ 0.75f, 1.0f},
        (RL_Vector2){ 1.0f, 0.75f},
        (RL_Vector2){ 1.0f, 0.5f},
        (RL_Vector2){ 0.75f, 0.0f}  // Close the poly
    };

    // Define the base poly vertices from the UV's
    // NOTE: They can be specified in any other way
    RL_Vector2 points[MAX_POINTS] = { 0 };
    ...
    RL_Vector2 positions[MAX_POINTS] = { 0 };
    for (int i = 0; i < MAX_POINTS; i++) positions[i] = points[i];

    // Load texture to be mapped to poly
    RL_Texture texture = RL_LoadTexture("resources/cat.png");
    ...
    while (!RL_WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        RL_BeginDrawing();
            ...
            DrawTexturePoly(texture, (RL_Vector2){ RL_GetScreenWidth()/2.0f, RL_GetScreenHeight()/2.0f },
