...
int main(void)
{
    ...
    RL_InitWindow(screenWidth, screenHeight, "raylib [core] example - generate random values");

    // RL_SetRandomSeed(0xaabbccff);   // Set a custom random seed if desired, by default: "time(NULL)"

    int randValue = RL_GetRandomValue(-8, 5);   // Get a random integer number between -8 and 5 (both included)
    ...
    while (!RL_WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        {
            randValue = RL_GetRandomValue(-8, 5);
            framesCounter = 0;
        }
        //----------------------------------------------------------------------------------
        ...
        RL_BeginDrawing();
            ...
            RL_DrawText(RL_TextFormat("%i", randValue), 360, 180, 80, RL_LIGHTGRAY);
