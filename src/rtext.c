...
#include <string.h>         // Required for: strcmp(), strstr(), strcpy(), strncpy() [Used in RL_TextReplace()], sscanf() [Used in LoadBMFont()]
...
static RL_GlyphInfo *LoadFontDataBDF(const unsigned char *fileData, int dataSize, int *codepoints, int codepointCount, int *outFontSize);
#endif
...
extern void LoadFontDefault(void)
{
    ...
    defaultFont.glyphs = (RL_GlyphInfo *)RL_MALLOC(defaultFont.glyphCount*sizeof(RL_GlyphInfo));
...
RL_Font RL_LoadFontEx(const char *fileName, int fontSize, int *codepoints, int codepointCount)
{
    RL_Font font = { 0 };
...
RL_Font RL_LoadFontFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int fontSize, int *codepoints, int codepointCount)
{
    RL_Font font = { 0 };
    ...
    {
        font.glyphs = RL_LoadFontData(fileData, dataSize, font.baseSize, codepoints, font.glyphCount, FONT_DEFAULT);
    }
    else
#endif
#if defined(SUPPORT_FILEFORMAT_BDF)
...
bool RL_IsFontReady(RL_Font font)
{
...
RL_GlyphInfo *RL_LoadFontData(const unsigned char *fileData, int dataSize, int fontSize, int *codepoints, int codepointCount, int type)
{
    ...
    if (fileData != NULL)
    {
        ...
        if (stbtt_InitFont(&fontInfo, (unsigned char *)fileData, 0))     // Initialize font for data reading
        {
            ...
            chars = (RL_GlyphInfo *)RL_CALLOC(codepointCount, sizeof(RL_GlyphInfo));
...
//#define SUPPORT_FONT_ATLAS_SIZE_CONSERVATIVE
    ...
    // shapes and text can be backed into a single draw call: RL_SetShapesTexture()
...
RL_Vector2 RL_MeasureTextEx(RL_Font font, const char *text, float fontSize, float spacing)
{
    ...
    for (int i = 0; i < size;)
    {
        ...
        int next = 0;
        letter = RL_GetCodepointNext(&text[i], &next);
...
char *RL_TextReplace(const char *text, const char *replace, const char *by)
{
    ...
    replaceLen = RL_TextLength(replace);
    if (replaceLen == 0) return NULL;  // Empty replace causes infinite loop during count

    byLen = RL_TextLength(by);
    ...
    temp = result = (char *)RL_MALLOC(RL_TextLength(text) + (byLen - replaceLen)*count + 1);
...
const char *RL_TextJoin(const char **textList, int count, const char *delimiter)
{
    static char buffer[MAX_TEXT_BUFFER_LENGTH] = { 0 };
    memset(buffer, 0, MAX_TEXT_BUFFER_LENGTH);
    char *textPtr = buffer;
...
// REQUIRES: strstr()
int RL_TextFindIndex(const char *text, const char *find)
{
    int position = -1;
...
const char *RL_TextToUpper(const char *text)
{
    static char buffer[MAX_TEXT_BUFFER_LENGTH] = { 0 };
    memset(buffer, 0, MAX_TEXT_BUFFER_LENGTH);
...
// WARNING: Limited functionality, only basic characters set
const char *RL_TextToLower(const char *text)
{
    static char buffer[MAX_TEXT_BUFFER_LENGTH] = { 0 };
    memset(buffer, 0, MAX_TEXT_BUFFER_LENGTH);
...
// WARNING: Limited functionality, only basic characters set
const char *RL_TextToPascal(const char *text)
{
    static char buffer[MAX_TEXT_BUFFER_LENGTH] = { 0 };
    memset(buffer, 0, MAX_TEXT_BUFFER_LENGTH);
...
// WARNING: Limited functionality, only basic characters set
const char *RL_TextToSnake(const char *text)
{
    static char buffer[MAX_TEXT_BUFFER_LENGTH] = {0};
    memset(buffer, 0, MAX_TEXT_BUFFER_LENGTH);
...
// WARNING: Limited functionality, only basic characters set
const char *RL_TextToCamel(const char *text)
{
    static char buffer[MAX_TEXT_BUFFER_LENGTH] = {0};
    memset(buffer, 0, MAX_TEXT_BUFFER_LENGTH);
...
static RL_GlyphInfo *LoadFontDataBDF(const unsigned char *fileData, int dataSize, int *codepoints, int codepointCount, int *outFontSize)
{
    ...
    RL_GlyphInfo *charGlyphInfo = NULL; // Pointer to output glyph info (NULL if not set)
