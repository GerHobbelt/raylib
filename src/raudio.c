...
// Audio buffer struct
struct RL_rAudioBuffer {
    ma_data_converter converter;    // Audio data converter

    AudioCallback callback;         // Audio buffer callback for buffer filling on audio threads
    RL_rAudioProcessor *processor;     // Audio processor
    ...
    RL_rAudioBuffer *next;             // Next audio buffer on the list
    RL_rAudioBuffer *prev;             // Previous audio buffer on the list
};

// Audio processor struct
// NOTE: Useful to apply effects to an AudioBuffer
struct RL_rAudioProcessor {
    AudioCallback process;          // Processor callback function
    RL_rAudioProcessor *next;          // Next audio processor on the list
    RL_rAudioProcessor *prev;          // Previous audio processor on the list
};

#define AudioBuffer RL_rAudioBuffer    // HACK: To avoid CoreAudio (macOS) symbol collision
...
typedef struct AudioData {
    ...
    RL_rAudioProcessor *mixedProcessor;
} AudioData;
...
bool RL_IsWaveReady(RL_Wave wave)
{
    bool result = false;
...
bool RL_IsSoundReady(RL_Sound sound)
{
    bool result = false;
...
RL_Music RL_LoadMusicStreamFromMemory(const char *fileType, const unsigned char *data, int dataSize)
{
    ...
    else if ((strcmp(fileType, ".flac") == 0) || (strcmp(fileType, ".FLAC") == 0))
    {
        ...
        if (ctxFlac != NULL)
        {
            music.ctxType = MUSIC_AUDIO_FLAC;
            music.ctxData = ctxFlac;
            music.stream = RL_LoadAudioStream(ctxFlac->sampleRate, ctxFlac->bitsPerSample, ctxFlac->channels);
            music.frameCount = (unsigned int)ctxFlac->totalPCMFrameCount;
            music.looping = true;   // Looping enabled by default
            musicLoaded = true;
        }
...
bool RL_IsMusicReady(RL_Music music)
{
...
bool RL_IsAudioStreamReady(RL_AudioStream stream)
{
...
void RL_AttachAudioStreamProcessor(RL_AudioStream stream, AudioCallback process)
{
    ...
    RL_rAudioProcessor *processor = (RL_rAudioProcessor *)RL_CALLOC(1, sizeof(RL_rAudioProcessor));
    processor->process = process;

    RL_rAudioProcessor *last = stream.buffer->processor;
...
void RL_DetachAudioStreamProcessor(RL_AudioStream stream, AudioCallback process)
{
    ...
    RL_rAudioProcessor *processor = stream.buffer->processor;

    while (processor)
    {
        RL_rAudioProcessor *next = processor->next;
        RL_rAudioProcessor *prev = processor->prev;
...
void RL_AttachAudioMixedProcessor(AudioCallback process)
{
    ...
    RL_rAudioProcessor *last = AUDIO.mixedProcessor;
...
void RL_DetachAudioMixedProcessor(AudioCallback process)
{
    ...
    RL_rAudioProcessor *processor = AUDIO.mixedProcessor;
            ...
            while (1)
            {
                ...
                while (framesToRead > 0)
                {
                    ...
                    if (framesJustRead > 0)
                    {
                        ...
                        RL_rAudioProcessor *processor = audioBuffer->processor;
                        while (processor)
                        {
...
static const char *RL_GetFileNameWithoutExt(const char *filePath)
{
    ...
    if (filePath != NULL) strcpy(fileName, RL_GetFileName(filePath));   // Get filename with extension
